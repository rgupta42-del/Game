...
(() => {
  ...
  $("share-btn").addEventListener("click", async () => {
    ...
    // Inside the iOS app (Capacitor), use the native share sheet — the Web
    // Share API isn't available in WKWebView.
    const capShare = window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.Share;
    if (capShare) {
      try { await capShare.share({ text }); return; } catch { /* user cancelled */ return; }
    }
    try {
